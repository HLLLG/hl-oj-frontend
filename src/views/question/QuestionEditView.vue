  ...
  <div class="edit-page">
    <div class="edit-container">
      <div class="page-header">
        ...
        <span class="page-id">ID: {{ questionId }}</span>
      </div>
...
import { getQuestionById, updateQuestion } from '@/api/questionController'
...
function safeParseJSON<T>(str?: string, fallback: T = [] as unknown as T): T {
...
async function loadQuestion() {
  ...
  try {
    const res: any = await getQuestionById({ id: questionId as any })
    if (res.data.code === 0 && res.data.data) {
      const q = res.data.data
      formState.title = q.title ?? ''
      formState.tags = safeParseJSON<string[]>(q.tags, [])
      formState.content = q.content ?? ''
      formState.answer = q.answer ?? ''
      formState.judgeCase = safeParseJSON<Array<{ input: string; output: string }>>(
        q.judgeCase,
        [{ input: '', output: '' }]
      )
      const cfg = safeParseJSON<any>(q.judgeConfig, {})
      formState.judgeConfig = {
        timeLimit: cfg.timeLimit ?? 1000,
        memoryLimit: cfg.memoryLimit ?? 262144,
        stackLimit: cfg.stackLimit ?? 262144,
      }
    } else {
      message.error(res.data.message || '加载题目失败')
